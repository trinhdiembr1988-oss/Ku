...
l)ocal FishReplicated = ReplicatedStorage:WaitForChild("FishReplicated")
...
-- ===== ADMIN TAB (PHẢI Ở ĐÂY) =====
local AdminTab = Window:AddTab("Admin")
local AdminBox = AdminTab:AddLeftGroupbox("Admin Panel")

local KamuiLoaded = false

AdminBox:AddButton({
    Title = "Kamui by Phuoc (OFF)",
    Callback = function()
        if not KamuiLoaded then
            KamuiLoaded = true

            task.spawn(function() -- fix không chạy
